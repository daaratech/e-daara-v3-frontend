    ...
    <template v-else>
        <v-app-bar flat border="b-sm" class="py-1">
            ...
            <template v-slot:extension>
                ...
                <v-fab
                v-if="getTabFormalTitle !== 'Emploi du temps'"
                    elevation="2"
                    rounded
                    color="blue"
                    class="text-subtitle-1 me-4 mt-15"
                    :text="'Ajouter'+ ' ' + getTabFormalTitle"
                    location="bottom end"
...
export default {
        ...
        formData: {
            fileNumber: {
                ...
                rules: [
                    ...
                    (v) =>
                        /^[A-Za-z0-9]+$/.test(v) ||
                        'Le numéro de dossier doit être alphanumérique.',
                    (v) =>
                        (v && v.length === 8) ||
                        'Le numéro de dossier doit comporter exactement 8 caractères.'
                ]
            },
            firstName: {
                label: 'Prénom',
            ...
            birthPlace: {
                ...
                rules: [
                    ...
                    (v) =>
                        (v && v.length >= 2) ||
                        'Le lieu de naissance doit comporter au moins 2 caractères.'
                ]
            },
            sexe: {
                label: 'Sexe',
                ...
                rules: [
                    ...
                    (v) =>
                        ['Masculin', 'Féminin'].includes(v) ||
                        'Le sexe doit être l’une des options valides.'
                ]
            },
            address: {
                label: 'Adresse',
            ...
            motherFirstName: {
                ...
                rules: [
                    ...
                    (v) =>
                        (v && v.length >= 5) ||
                        'Le nom complet de la mère doit comporter au moins 5 caractères.',
                    (v) =>
                        (v && v.length <= 100) ||
                        'Le nom complet de la mère ne doit pas dépasser 100 caractères.'
                ]
            },
            tutorFullName: {
                ...
                rules: [
                    ...
                    (v) =>
                        (v && v.length >= 5) ||
                        'Le nom complet du tuteur doit comporter au moins 5 caractères.',
                    (v) =>
                        (v && v.length <= 100) ||
                        'Le nom complet du tuteur ne doit pas dépasser 100 caractères.'
                ]
            },
            tutorPhone: {
                ...
                rules: [
                    ...
                    (v) =>
                        /^\d{10}$/.test(v) ||
                        'Le numéro de téléphone du tuteur doit comporter 10 chiffres.'
                ]
            }
        }
    }),
    ...
    computed: {
        getTabFormalTitle() {
            return this.tab === 'tab-student'
                ? 'Etudiant'
                : this.tab === 'tab-professor'
                  ? 'Professeur'
                  : this.tab === 'tab-course'
                    ? 'Cours'
                    : this.tab === 'tab-agenda'
        ...
        // },
        dateFieldComputed() {
            return this.formData.birthDate.value
                ? this.$longDateFormat(this.formData.birthDate.value)
                : '';
        }
    }
};
